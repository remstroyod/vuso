...
class FaqPolicy
{
     ...
     *
     * @param  \Backend\Models\Profile\User  $user
     * @param  \Backend\Models\Faq  $faq
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function view(User $user, Faq $faq)
    {
        return ($user->can('faq_access')|| $user->hasRole('admin'))
            ? Response::allow()
    ...
    {
        return ($user->can('faq_create')|| $user->hasRole('admin'))
            ? Response::allow()
            : Response::deny(__('Нет доступа'));
    }
    ...
    {
        return ($user->can('faq_update')|| $user->hasRole('admin'))
            ? Response::allow()
            : Response::deny(__('Нет доступа'));
    }
    ...
    {
        return ($user->can('faq_destroy')|| $user->hasRole('admin'))
            ? Response::allow()
            : Response::deny(__('Нет доступа'));
    }
